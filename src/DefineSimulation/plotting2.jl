    ...
    if by==nothing
        ...
        if iszero(N)
            ...
            for i ∈ 1:N
                ...
                renorm = (maximum(abs.(ψ[.!isnan.(ψ)])) - minimum(abs.(ψ[.!isnan.(ψ)])))/(maximum(n₁[.!isnan.(n₁)])-minimum(n₁[.!isnan.(n₁)]))

                n₁ = n₁ .- minimum(n₁[.!isnan.(n₁)])
                n₁ = renorm*n₁
                @series begin
                    subplot := i
                    seriestype := :heatmap
                ...
                @series begin
                    ...
                    alpha --> .5
                    if by ∈ [abs, abs2]
                        clims --> (0, maximum(by.(ψ[.!isnan.(ψ)])))
                    else
                        clims --> (-maximum(abs.(ψ[.!isnan.(ψ)])),+maximum(abs.(ψ[.!isnan.(ψ)])))
                    end
                    x_t, y_t, permutedims(by.(ψ./maximum(abs.(ψ[.!isnan.(ψ)]))))
                end
            end
        end
    end
